/**
 * fireworks.js
 * Isolated Pyrotechnic Engine (Based on Caleb Miller's CodePen)
 * Modified for Pomodoro Forest App integration.
 */
...
(function() {
        ...
        constructor(id) {
            ...
            if (!this.canvas) {
                ...
                if (id === 'trails-canvas') {
                    ...
                    this.canvas.style.zIndex = '99997'; // Behind main
                }
    ...
    function randomColor(options) {
        ...
        if (options && options.notSame) while (color === lastColor) color = randomColorSimple();
        else if (options && options.notColor) while (color === options.notColor) color = randomColorSimple();
        lastColor = color;
        return color;
    }
    ...
    function createBurst(count, particleFactory, startAngle=0, arcLength=PI_2) {
        ...
        for (let i=0; i<=C_HALF; i++) {
            ...
            for (let i=0; i<partsPerArc; i++) {
                let angle = angleInc * i + angleOffset + (Math.random() * angleInc * 0.33);
    ...
    const crysanthemumShell = (size=1) => {
        ...
        return { shellSize: size, spreadSize: 300 + size * 100, starLife: 900 + size * 200, starDensity: 1.25, color, pistil, pistilColor: pistil && makePistilColor(color), streamers: !pistil && color !== COLOR.White && Math.random() < 0.42 };
    };
    ...
    const ghostShell = (size=1) => {
        const shell = crysanthemumShell(size); shell.starLife *= 1.5; let ghostColor = randomColor({ notColor: COLOR.White });
        shell.streamers = true; const pistil = Math.random() < 0.42; shell.pistilColor = pistil && makePistilColor(ghostColor);
    ...
    const SHELL_TYPES = [crysanthemumShell, ringShell, ghostShell, strobeShell, palmShell, crossetteShell, floralShell, fallingLeavesShell, willowShell, crackleShell, horsetailShell];
    function randomFastShell() { return SHELL_TYPES[Math.floor(Math.random() * SHELL_TYPES.length)]; }
    ...
    class Shell {
        ...
        launch(position, launchHeight) {
            ...
            const comet = this.comet = Star.add(
                launchX, launchY, typeof this.color === 'string' && this.color !== 'random' ? this.color : COLOR.White,
                Math.PI, launchVelocity * (this.horsetail ? 1.2 : 1), launchVelocity * (this.horsetail ? 100 : 400)
            );
            comet.heavy = true;
        ...
        burst(x, y) {
            ...
            const starFactory = (angle, speedMult) => {
                ...
                const star = Star.add(x, y, color || randomColor(), angle, speedMult * speed,
                    this.starLife + Math.random() * this.starLife * this.starLifeVariation, 0, -standardInitialSpeed);
                if (this.secondColor) {
            ...
            if (typeof this.color === 'string') {
                ...
                if (this.ring) {
                    ...
                    createParticleArc(0, PI_2, this.starCount, 0, angle => {
                        ...
                        if (this.glitter) {
                            star.sparkFreq = sparkFreq; star.sparkSpeed = sparkSpeed; star.sparkLife = sparkLife;
                            star.sparkLifeVariation = sparkLifeVariation; star.sparkColor = this.glitterColor; star.sparkTimer = Math.random() * star.sparkFreq;
                        }
                    });
                } else {
            ...
            if (this.pistil) {
                const innerShell = new Shell({ spreadSize: this.spreadSize * 0.5, starLife: this.starLife * 0.6, starLifeVariation: this.starLifeVariation, starDensity: 1.4, color: this.pistilColor, glitter: 'light', glitterColor: this.pistilColor === COLOR.Gold ? COLOR.Gold : COLOR.White });
                innerShell.burst(x, y);
            }
            
            if (this.streamers) {
                const innerShell = new Shell({ spreadSize: this.spreadSize * 0.9, starLife: this.starLife * 0.8, starLifeVariation: this.starLifeVariation, starCount: Math.floor(Math.max(6, this.spreadSize / 45)), color: COLOR.White, glitter: 'streamer' });
                innerShell.burst(x, y);
            }
    ...
    function seqSmallBarrage() {
        ...
        function launchShell(x) {
            const shell = new Shell(randomFastShell()(shellSize));
    ...
    function update() {
        ...
        // Cap frameTime to avoid big jumps
        const timeStep = Math.min(frameTime, 50); 
        const speed = 1; // simSpeed
        const lag = frameTime / 16.66;
        ...
        COLOR_CODES_W_INVIS.forEach(color => {
            ...
            for (let i=stars.length-1; i>=0; i=i-1) {
                ...
                star.life -= timeStep;
                if (star.life <= 0) {
                    stars.splice(i, 1); Star.returnInstance(star);
                } else {
            ...
            for (let i=sparks.length-1; i>=0; i=i-1) {
                ...
                spark.life -= timeStep;
                if (spark.life <= 0) {
                    sparks.splice(i, 1); Spark.returnInstance(spark);
                } else {
    ...
    // ── PUBLIC API ────────────────────────────────────────────────────────────
    window.fireFireworksEffect = function(cx, cy, container, isLocal) {
        if (!trailsStage) trailsStage = new Stage('trails-canvas');
        if (!mainStage) mainStage = new Stage('main-canvas');
        ...
        trailsStage.canvas.style.display = 'block';
        mainStage.canvas.style.display = 'block';
        ...
        if (isLocal) {
            // Local Preview: Random shell locally
            const shell = new Shell(randomFastShell()(3));
            shell.launch(0.5, 0.5);
            runTimeout = setTimeout(() => {
                isRunning = false;
                trailsStage.ctx.clearRect(0, 0, stageW, stageH);
                mainStage.ctx.clearRect(0, 0, stageW, stageH);
            }, 5000);
        } else {
            // Full Finale
            seqSmallBarrage();
            setTimeout(() => { seqSmallBarrage(); }, 2500);
